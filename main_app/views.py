  ...
  if request.method == 'GET':
     ...
     return render(request,'patient/checkdisease/checkdisease.html', {"list2":alphabaticsymptomslist})
      ...
      if (inputno == 0 ) :
        ...
        predicted, confidencescores = final.naivebayes(inputtest)
        print(predicted)
        print(int(confidencescores))
...
def dviewprofiles(request, doctorusername):
         duser = User.objects.get(username=doctorusername)
         r = rating_review.objects.filter(doctor=duser.doctor)
       
         return render(request,'doctor/view_profile1/doc.html', {"duser":duser, "rate":r} )




def dviewprofiless(request, doctorusername):
         duser = User.objects.get(username=doctorusername)
         r = rating_review.objects.filter(doctor=duser.doctor)
       
         return render(request,'doctor/view_profile2/doc.html', {"duser":duser, "rate":r} )
...
def close_doctor_consultation(request,consultation_id):
   consultation.objects.filter(pk=consultation_id).update(status="closed")
