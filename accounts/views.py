...
def signup_patient(request):
    ...
    if request.method == 'POST':
      ...
      if request.POST['username'] and request.POST['email'] and  request.POST['name'] and request.POST['gender'] and request.POST['password']and request.POST['password1'] :
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                patientnew = patient(user=user,name=name,gender=gender)
                patientnew.save()
...
def savepdata(request,patientusername):
  ...
  if request.method == 'POST':
    ...
    patient.objects.filter(pk=puser.patient).update(name=name,gender=gender)
